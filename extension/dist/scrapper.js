(() => {
  ...
  // src/gig.js
  var convertmd = new TurndownService();
    ...
    try {
      packageTabs.forEach((tab) => {
        ...
        packages[tab.textContent.trim()] = {
          ...
          "includes": [...content.querySelectorAll("div.collapsable-package-item div.collapsible-content ul li")].filter((li) => li.querySelector("span")?.classList.contains("ff7c5d1")).map((li) => li.textContent.trim())
        };
      });
    } catch (e) {
  ...
  function getRatings() {
    ...
    ratings.querySelectorAll("li").forEach((rating) => {
      let key = rating.childNodes[0].textContent.trim().replace(":", "").replaceAll(" ", "_");
